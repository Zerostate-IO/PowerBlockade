...
(Nodes → Add Node → Generate Deployment Package):
...
The generated package runs only the DNS services (dnsdist, recursor,
recursor-reloader, dnstap-processor) and sync-agent. It does **not** run
postgres, admin-ui, prometheus, or grafana — those run only on the primary.

> ⚠️ A repo clone plus `docker compose --profile secondary up -d` does **not**
> produce a thin secondary: only `sync-agent` is profile-gated in the canonical
> compose files, so postgres, admin-ui, prometheus, and grafana start anyway.
