...
Secondary nodes use the **thin package generated by the primary's Admin UI**
(Nodes → Add Node → Generate Deployment Package). The generated package
contains only the DNS services (dnsdist, recursor, recursor-reloader,
dnstap-processor) and sync-agent:
...
> A raw repo clone plus `--profile secondary` is **not** a thin secondary: the
> canonical compose files only profile-gate `sync-agent`, so postgres,
> admin-ui, prometheus, and grafana would also start.
