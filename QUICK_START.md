...
Use the **thin package generated by the primary's Admin UI** (Nodes → Add Node →
Generate Deployment Package) — a repo clone runs the full primary stack because
only `sync-agent` is profile-gated in the canonical compose files:

```bash
# On the secondary node, unpack the generated package and start it
